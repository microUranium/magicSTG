...
func create_phase1_pattern() -> Array[AttackPattern]:
  ...
  var angle_spreads = [90, 3877.5, 90, 3877.5, 3877.5, 90, 3877.5, 90]
  var angle_offsets = [45, 225, 45, 225, 135, 315, 135, 315]

  for i in range(4):
    var pattern = AttackPatternFactory.create_rapid_fire(
      AttackPatternFactory.DEFAULT_BULLET_SCENE, 12, 0.05, 3
    )
    pattern.target_group = "players"
    pattern.direction_type = AttackPattern.DirectionType.FIXED
    pattern.angle_spread = angle_spreads[i]
    pattern.angle_offset = angle_offsets[i]

    patterns.append(pattern)

  var layered_pattern = AttackPatternFactory.create_layered_pattern(patterns, [0.0, 0.0, 0.0, 0.0])
  layered_pattern.burst_delay = 3
  ...
  layered_pattern = AttackPatternFactory.create_layered_pattern(patterns, [0.0, 0.0, 0.0, 0.0])
  layered_pattern.burst_delay = 3
