...
func fade_out():
  ...
  tw.parallel().tween_property(
    _face_left, "position", _left_home + Vector2(-slide_distance_face, 0), fade_time
  )
  tw.parallel().tween_property(
    _face_right, "position", _right_home + Vector2(slide_distance_face, 0), fade_time
  )

  if _box.scale.x > 0:  # フキダシの向きが左側
    tw.parallel().tween_property(
      _box, "position", _box_home + Vector2(0, slide_distance_box), fade_time
    )
  else:  # フキダシの向きが右側
    tw.parallel().tween_property(
      _box,
      "position",
      Vector2(-_box_home.x, _box_home.y) + Vector2(0, slide_distance_box),
      fade_time
    )
  await tw.finished
